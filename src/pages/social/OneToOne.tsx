...
const OneToOne = () => {
  ...
  useEffect(() => {
    ...
    const fallback = roommates.find(
      (roommate) => roommate.id !== currentUser.id,
    );
  ...
  const selectedRoommate =
    otherRoommates.find((roommate) => roommate.id === selectedRoommateId) ??
    null;
  ...
  const sharedExpenses = useMemo(() => {
    ...
    return approvedExpenses.filter(
      (expense) =>
        expense.addedById === currentUser.id ||
        expense.addedById === selectedRoommateId,
    );
  }, [approvedExpenses, currentUser, selectedRoommateId]);

  const balance = currentUserTotal - selectedRoommateTotal;
  const comparisonMessage = !selectedRoommate
        ...
        <CardContent className="space-y-3">
          ...
          {selectedRoommate && sharedExpenses.length === 0 && (
            <p className="text-sm text-muted-foreground">
              No approved expenses between you yet.
            </p>
          )}
          {!selectedRoommate && (
            <p className="text-sm text-muted-foreground">
              Select a roommate to see shared expenses.
            </p>
          )}
        </CardContent>
      </Card>
