...
const Expenses = () => {
  ...
  const categoryOptions = useMemo(() => {
    return Array.from(
      new Set(approvedExpenses.map((expense) => expense.category)),
  ...
  const filteredExpenses = useMemo(() => {
    ...
    return approvedExpenses.filter((expense) => {
      const matchesRoommate =
        roommateFilter === "all" ||
        (expense.addedByName || "Unknown") === roommateFilter;
      ...
      const matchesAmount =
        ...
        expense.amount.toFixed(2).includes(normalizedAmount);
      const expenseDate = parseExpenseDate(expense.date);
      const matchesMonth = expenseDate
        ? getMonthKey(expenseDate) === getMonthKey(monthFilter)
        : false;
      return (
        matchesRoommate &&
        matchesCategory &&
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    {monthFilter
                      ? monthFilter.toLocaleString("en-US", {
                          month: "short",
                          year: "numeric",
                        })
                      : "Pick a month"}
                ...
                <TableHead className="h-auto py-3">
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={setRoommateFilter}
                    >
                      <SelectTrigger className="h-8 min-w-[160px]">
